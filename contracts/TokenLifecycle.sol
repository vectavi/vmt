...
contract TokenLifecycle {
    ...
    function setPhase(Phase _nextPhase) public onlyTokenManager {
        bool canSwitchPhase;
        if (currentPhase == Phase.Created && _nextPhase == Phase.Running) {
            canSwitchPhase = true;
        } else {
            if (currentPhase == Phase.Running && _nextPhase == Phase.Paused) {
                canSwitchPhase = true;
            } else {
                if (currentPhase == Phase.Paused && _nextPhase == Phase.Running) {
                    canSwitchPhase = true;
                } else {
                    if (currentPhase == Phase.Running && _nextPhase == Phase.Finalizing) {
                        canSwitchPhase = true;
                    } else {
                        if (currentPhase == Phase.Paused && _nextPhase == Phase.Finalizing) {
                            canSwitchPhase = true;
                        } else {
                            if (currentPhase == Phase.Refunding && _nextPhase == Phase.Finalized) {
                                canSwitchPhase = true;
                            } else {
                                // switch to migration phase only if migration manager is set
                                if (currentPhase == Phase.Finalized && _nextPhase == Phase.Migrating && 
                                    migrationManager != 0x0) {
                                    canSwitchPhase = true;
                                } else {
                                    // override switch to migrated when migration is held up
                                    // - should not be needed in normal use case
                                    if (currentPhase == Phase.Migrating && _nextPhase == Phase.Migrated) {
                                        canSwitchPhase = true;
                                    } else {
