...
const VestedTokenMock = artifacts.require("./helpers/VestedTokenMock.sol");


contract('VestedToken', function(accounts) {
  const receiver = accounts[1];
  const tokenManager = accounts[9];
  const granter = tokenManager; // only token manager is allowed to grant tokens
  ...
  beforeEach(async () => {
    token = await VestedTokenMock.new(tokenManager, granter, initialSupply);
  ...
  it('granter can grant tokens without vesting', async () => {
    ...
    assert.equal(await token.xferableTokens(receiver, now), tokenAmount);
  })
  ...
  describe('getting a revokable/non-burnable token grant', async () => {
    ...
    it('has 0 transferable tokens before cliff', async () => {
      assert.equal(await token.xferableTokens(receiver, now), 0);
    })

    it('all tokens are transferable after vesting', async () => {
      assert.equal(await token.xferableTokens(receiver, now + vesting), tokenAmount);
    })
    ...
    it('can handle composed vesting schedules', async () => {
      ...
      assert.equal(await token.xferableTokens(receiver, newNow), 0)
      await timer(vesting);
